...
const cooldown = 600000 //25 minutos
...
𝙿𝙾𝚁 𝙵𝙰𝚅𝙾𝚁 𝙲𝚄𝚁𝙰𝚃𝙴 𝙿𝚁𝙸𝙼𝙴𝚁𝙾 𝙿𝙰𝚁𝙰 𝙰𝚃𝙰𝙲𝙰𝚁 𝙳𝙴 𝙽𝚄𝙴𝚅𝙾`.trim(), imgr + 'MALA SALUD', [
...
`𝚈𝙰 𝙰𝚃𝙰𝙲𝙰𝚂𝚃𝙴 𝚈 𝙼𝙰𝚃𝙰𝚂𝚃𝙴 𝚄𝙽𝙰 𝙲𝙸𝚄𝙳𝙰𝙳 𝙴𝙽𝚃𝙴𝚁𝙰 𝙿𝙾𝚁 𝙵𝙰𝚅𝙾𝚁 𝙴𝚂𝙿𝙴𝚁𝙰 𝙷𝙰𝚂𝚃𝙰 𝚀𝚄𝙴 𝚃𝙴𝚁𝙼𝙸𝙽𝙴 𝙴𝙻 𝚃𝙸𝙴𝙼𝙿𝙾 𝙳𝙴 𝙳𝙴𝚂𝙲𝙰𝙽𝚂𝙾

⏱️ ${timers.toTimeString()}`.trim(), imgr + 'DESCANSANDO', [
[`🔔 𝚁𝙴𝙲𝙻𝙰𝙼𝙾 𝙳𝙸𝙰𝚁𝙸𝙾`, `${usedPrefix}daily`]], fkontak, m) 
    
const rewards = reward(user)
let text = `⚔️ 𝙴𝚂𝚃𝙰𝚂 𝙰𝚃𝙰𝙲𝙰𝙽𝙳𝙾 𝙴𝙽  *» ${kt[1][0].name}*
...
handler.tags = ['rpg']
handler.command = /^(atacar|matanza)$/i
...
function reward(user = {}) {
    let rewards = {
	    ...
	    joincount: 2,
            ...
            uncoommon: [2, 9, 7, 1, 9].concat(
                new Array(5 - (
                    (user.dog > 2 && user.dog < 6 && user.dog) || (user.dog > 5 && 5) || 2
                )).fill(0)
            ),
            mythic: [1, 1, 0, 0, 0, 1, 0, 0, 0].concat(
                new Array(8 - (
        ...
        lost: {
            ...
            bowdurability: (15 - user.bow) * 7
            sworddurability: (15 - user.sword) * 7
        }
    }
    return rewards
}